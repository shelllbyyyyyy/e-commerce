...
import {
  Subscription,
  SubscriptionUpdate,
} from './dto/subscription/Subscription';
...
export class MidtransService extends BaseService {
  ...
  async unbindGopayAccount(accountId: string) {
    const data = await this.handleRequest(
      'post',
      `/v2/pay/account/${accountId}/unbind`,
    );
    return data;
  }

  async getGopayPromotion(
    accountId: string,
    grossAmount: string,
    currency: string = 'IDR',
  ) {
  ...
  async validateSignature(
    signatureKey: string,
    data: { orderId: string; statusCode: string; grossAmount: string },
  ) {
    const hash = crypto.createHash('sha512');
    const hashed = hash.update(
      data.orderId + data.statusCode + data.grossAmount + this.config.serverKey,
    );
  ...
  async disableSubscription(subscriptionId: string) {
    const data = await this.handleRequest(
      'post',
      `/v1/subscriptions/${subscriptionId}/disable`,
    );
    return data;
  }
  ...
  async enableSubscription(subscriptionId: string) {
    const data = await this.handleRequest(
      'post',
      `/v1/subscriptions/${subscriptionId}/enable`,
    );
    return data;
  }
